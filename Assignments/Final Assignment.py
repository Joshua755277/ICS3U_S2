...
FullName = []
# Stores full names (first and last) of all cardholders
CCTypes = []
...
def validate(N):
    N = reverse_string(N)
    # Reverse the credit card number for right-to-left processing
    total = 0
    # Accumulates the Luhn checksum
    for i in range(len(N)):
        digit = int(N[i])
        ...
        if i % 2 == 1:
            ...
            total += digit
            # Add the digit as-is if not one being doubled
    return total % 10 == 0
    # Return True if total is divisible by 10 (valid)
...
def merge(arr, arr2, arr3, arr4, l, m, r):
    ...
    L = [0] * (n1); L2 = [0] * (n1); L3 = [0] * (n1); L4 = [0] * (n1)
    R = [0] * (n2); R2 = [0] * (n2); R3 = [0] * (n2); R4 = [0] * (n2)
    # Temporary arrays for merging each parallel list

    for i in range(n1):
        L[i] = arr[l + i]; L2[i] = arr2[l + i]; L3[i] = arr3[l + i]; L4[i] = arr4[l + i]
        # Copy left half into temporary arrays
    for j in range(n2):
        R[j] = arr[m + 1 + j]; R2[j] = arr2[m + 1 + j]; R3[j] = arr3[m + 1 + j]; R4[j] = arr4[m + 1 + j]
    ...
    while i < n1 and j < n2:
        if L[i] <= R[j]:
            arr[k] = L[i]; arr2[k] = L2[i]; arr3[k] = L3[i]; arr4[k] = L4[i]
            i += 1
        else:
            arr[k] = R[j]; arr2[k] = R2[j]; arr3[k] = R3[j]; arr4[k] = R4[j]
            j += 1
        k += 1
...
try:
    ...
    while not EndOfFile:
        line = f.readline().strip()
        # Read a line and strip whitespace
        EndOfFile = (line == "")
...
for x in arr:
    ...
    # Split data fields
    Name = GivenName + ' ' + Surname
    FullName.append(Name); CCTypes.append(CCType); CCNumbers.append(CCNumber)
    # Store name, card type, and number
    if len(Exp_Mo) == 1:
        Exp_Mo = '0' + Exp_Mo
        # Pad single-digit month
    Expiry_Date = Exp_Yr + Exp_Mo
    Expiry_Dates.append(int(Expiry_Date))
    # Store formatted expiry date as integer
...
for i in range(len(Expiry_Dates)):
    ...
    print("%-37s %-15s %-20s %-10s %-25s" % 
          (FullName[i] + ':', CCTypes[i], '#' + CCNumbers[i], Expiry_Dates[i], Expired_Text + " | " + validity))
    # Display the result formatted for screen
